	...
	*/
	function skematik_shortcode_progress_bar( $atts, $content = null ) {
	  ...
	  'goal' => '',
	    ...
	    $mywidth = do_shortcode('[cfdb-count form="'.$form.'" filter="'.$filter.'"]');
	    $width = ($mywidth/$goal) * 100;
	  }

		$output = '<div class="progress '.$type.' '.$striped.' '.$animate.'"><div class="progress-bar" style="width:'.$width.'%;">&nbsp;'.$mywidth.'/'.$goal.'</div></div>';
		return $output;}
