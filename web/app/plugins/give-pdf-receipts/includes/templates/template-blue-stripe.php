...
function give_pdf_template_blue_stripe( $give_pdf, $give_pdf_payment, $give_pdf_payment_meta, $give_pdf_buyer_info, $give_pdf_payment_gateway, $give_pdf_payment_method, $address_line_2_line_height, $company_name, $give_pdf_payment_date, $give_pdf_payment_status ) {
	...
	$pdf_name                   = isset( $give_options['give_pdf_name'] ) ? $give_options['give_pdf_name'] : '';
	$pdf_address_l1             = isset( $give_options['give_pdf_address_line1'] ) ? $give_options['give_pdf_address_line1'] : '';
	$pdf_address_l2             = isset( $give_options['give_pdf_address_line2'] ) ? $give_options['give_pdf_address_line2'] : '';
	$pdf_address_city_state_zip = isset( $give_options['give_pdf_address_city_state_zip'] ) ? $give_options['give_pdf_address_city_state_zip'] : '';

	$font  = isset( $give_options['give_pdf_enable_char_support'] ) ? 'droidsansfallback' : 'droidserif';
	$fontb = isset( $give_options['give_pdf_enable_char_support'] ) ? 'droidsansfallback' : 'droidserifb';

	$give_pdf->AddFont( $font, '' );
	$give_pdf->AddFont( $fontb, '' );
	...
	$give_pdf->SetX( 35 );
	$give_pdf->SetFont( $fontb, '', 10 );
	$give_pdf->Cell( 33, 6, __( 'Payment Method', 'give_pdf' ), 0, 0, 'L', false );
	$give_pdf->SetFont( $font, '', 10 );
	$give_pdf->Cell( 0, 6, $give_pdf_payment_method, 0, 2, 'L', false );

	$line_height_totals = give_pdf_calculate_line_height( $pdf_name ) +
	                      give_pdf_calculate_line_height( $pdf_address_l1 ) +
	                      give_pdf_calculate_line_height( $pdf_address_l2 ) +
	                      give_pdf_calculate_line_height( $pdf_address_city_state_zip ) +
	                      give_pdf_calculate_line_height( $give_options['give_pdf_email_address'] );

	$give_pdf->SetXY( 150, 77 );
	$give_pdf->SetFillColor( 149, 210, 236 );

	if ( isset( $give_options['give_pdf_url'] ) && $give_options['give_pdf_url'] ) {
		$give_pdf->Rect( 203, 76.5, 0.5, $line_height_totals + 6, 'F' );
	} else {
		$give_pdf->Rect( 203, 76.5, 0.5, $line_height_totals, 'F' );
	} // end if

	$give_pdf->SetFont( $font, '', 9 );

	if ( ! empty( $pdf_name ) ) {
		$give_pdf->Cell( 0, give_pdf_calculate_line_height( $pdf_name ), give_pdf_get_settings( $give_pdf, 'name' ), 0, 2, 'R', false );
	} // end if

	if ( ! empty( $pdf_address_l1 ) ) {
		$give_pdf->Cell( 0, give_pdf_calculate_line_height( $pdf_address_l1 ), give_pdf_get_settings( $give_pdf, 'addr_line1' ), 0, 2, 'R', false );
	} // end if

	if ( ! empty( $pdf_address_l2 ) ) {
		$give_pdf->Cell( 0, give_pdf_calculate_line_height( $pdf_address_l2 ), give_pdf_get_settings( $give_pdf, 'addr_line2' ), 0, 2, 'R', false );
	} // end if

	if ( ! empty( $pdf_address_city_state_zip ) ) {
		$give_pdf->Cell( 0, give_pdf_calculate_line_height( $pdf_address_city_state_zip ), give_pdf_get_settings( $give_pdf, 'city_state_zip' ), 0, 2, 'R', false );
	} // end if
	...
	if ( $give_pdf_payment_meta ) {
		...
		$give_pdf_total_price = html_entity_decode( give_currency_filter( give_format_amount( give_get_payment_amount( $payment_id ) ) ), ENT_COMPAT, 'UTF-8' );
		...
		$give_pdf->MultiCell( 122, $linecount * 5, $give_pdf_donation_title, 0, 'L', false, 0, 35 );
		$give_pdf->Cell( 45, $linecount * 5, $give_pdf_total_price, 0, 2, 'R', false );
		...
		$give_pdf->SetX( 35 );
		$give_pdf->Cell( 0, 11, __( 'Total Donation', 'give_pdf' ) . ' - ' . $total, 0, 2, 'L', false );
