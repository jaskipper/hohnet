<?php
/**
 * Minimal PDF Receipt Template
 *
 * Builds and renders the minimal PDF receipt template .
 *
 * @since 1.0
 *
 * @uses  HTML2PDF
...
function give_pdf_template_minimal( $give_pdf, $give_pdf_payment, $give_pdf_payment_meta, $give_pdf_buyer_info, $give_pdf_payment_gateway, $give_pdf_payment_method, $address_line_2_line_height, $company_name, $give_pdf_payment_date, $give_pdf_payment_status ) {
	global $give_options;

	$give_pdf->AddFont( 'droidserif', '' );

	$font  = isset( $give_options['give_pdf_enable_char_support'] ) ? 'droidsansfallback' : 'droidserif';
	$fontb = isset( $give_options['give_pdf_enable_char_support'] ) ? 'droidsansfallback' : 'Helvetica';

	$give_pdf->SetMargins( 20, 20, 20 );
	$give_pdf->SetX( 8 );

	$give_pdf->AddPage();

	$give_pdf->SetFont( $fontb, '', 12 );
	...
	$give_pdf->SetTextColor( 222, 59, 22 );
	...
	$give_pdf->SetXY( 20, 55 );

	$give_pdf->SetFont( $fontb, 'B', 10 );
	$give_pdf->Cell( 33, 6, __( 'Transaction ID', 'give_pdf' ), 0, 0, 'L', false );
	$give_pdf->SetFont( $fontb, '', 10 );
	$give_pdf->Cell( 0, 6, give_pdf_get_payment_number( $give_pdf_payment->ID ), 0, 2, 'L', false );
	$give_pdf->SetX( 20 );
	$give_pdf->SetFont( $fontb, 'B', 10 );
	$give_pdf->Cell( 33, 6, __( 'Donation Key', 'give_pdf' ), 0, 0, 'L', false );
	$give_pdf->SetFont( $fontb, '', 10 );
	...
	$give_pdf->Rect( 20, 132, 170, 0.5, 'F' );
	...
	if ( $give_pdf_payment_meta ) {
		...
		$give_pdf->MultiCell( 122, $linecount * 4, $give_pdf_donation_title, 0, 'L', false, 0, 20 );
		$give_pdf->Cell( 45, $linecount * 4, $give_pdf_total_price, 0, 2, 'R', false );
		...
		if ( ! empty ( $fees ) ) :
			foreach ( $fees as $fee ) :
				$give_pdf->SetX( 35 );
				$give_pdf->Cell( 0, 8, $fee['label'] . ' - ' . html_entity_decode( give_currency_filter( $fee['amount'] ), ENT_COMPAT, 'UTF-8' ), 0, 2, 'R', false );
			endforeach;
		endif;

		$give_pdf->SetX( 35 );
		$give_pdf->Cell( 0, 11, __( 'Total Donation', 'give_pdf' ) . ' - ' . $give_pdf_total_price, 0, 2, 'R', false );

		$give_pdf->Ln( 10 );

		if ( isset ( $give_options['give_pdf_additional_notes'] ) && ! empty( $give_options['give_pdf_additional_notes'] ) ) {
			$give_pdf->SetX( 35 );
			$give_pdf->SetFont( $fontb, '', 13 );
			$give_pdf->SetTextColor( 224, 65, 28 );
...
add_action( 'give_pdf_template_minimal', 'give_pdf_template_minimal', 10, 10 );