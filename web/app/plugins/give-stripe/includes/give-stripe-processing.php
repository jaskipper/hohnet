		...
		try {
			...
			if ( class_exists( 'Give_Recurring' ) && Give_Recurring()->is_purchase_recurring( $purchase_data ) && ( ! empty( $customer ) || $customer_exists ) ) {
				...
				if ( ! isset( $give_options['stripe_preapprove_only'] ) ) {
					...
					$stripe_charge = apply_filters( 'give_stripe_charge_data', array(
							'amount'               => $amount,
							'currency'             => give_get_currency(),
				...
				give_stripe_preapproval_send_admin_notice( $payment );


			} else {

				give_record_gateway_error( __( 'Customer Creation Failed', 'give-stripe' ), sprintf( __( 'Customer creation failed while processing a payment. Payment Data: %s', 'give-stripe' ), json_encode( $payment_data ) ) );
