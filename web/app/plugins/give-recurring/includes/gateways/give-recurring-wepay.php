...
class Give_Recurring_WePay extends Give_Recurring_Gateway {
	...
	public function create_payment_profiles() {
		...
		$this->name = give_recurring_generate_subscription_name($this->subscriptions['id'], $this->subscriptions['price_id']);

		// Format a plan ID
		$this->plan_id = $this->name . '_' . $this->subscriptions['recurring_amount'] . '_' . $this->subscriptions['period'];
		...
		if ( isset( $_GET['subscription_id'] ) && $_GET['subscription_id'] ) {
			...
			try {
				...
				if ( isset( $wepay_subscription->state ) && $wepay_subscription->state == 'captured' ) {
					...
					$customer_id = give_get_payment_customer_id( $payment->parent_payment_id );
	 ...
	 *
	 * @description: This message displays when the user has completed an offsite transaction and has been redirected to the thank you page while the transaction is processing at WePay.
	 */
	public function output_pending_wepay_transaction_message() {

		give_output_error( __( 'We are still confirming the initial subscription charge with WePay. This can take a few minutes to complete.' ), true );
	...
	public function create_wepay_plan( $subscription = array() ) {
		...
		$description  = get_post_field( 'post_excerpt', $subscription['id'] );
		$form_content = get_post_meta( $subscription['id'], '_give_form_content', true );
		if ( ! empty( $form_content ) ) {
			$description = $form_content;
		} elseif ( empty( $description ) ) {
			$description = get_post_field( 'post_title', $subscription['id'] );
		}
	 ...
	 *
	 * @description: WePay supports a period for each subscription. Either "weekly", "monthly", "yearly", or "quarterly".
	...
	public function can_cancel( $ret, $subscription ) {
		...
		if ( $subscription->gateway === 'wepay' && ! empty( $subscription->profile_id ) && 'active' === $subscription->status ) {
			$ret = true;
		}
